    ...
    with open(file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            for gram_size in range(1, min(max_grams, len(tokens)) + 1):
                ...
                if not grams[gram_size]:
                    # no higher grams are possible
                    break
    return grams


def find_overlap(file1, file2, max_grams):
    """Computes overlap between n-grams"""
    mem1 = make_grams(file1, max_grams)
    mem2 = make_grams(file2, max_grams)
    ...
    for i, (set1, set2) in enumerate(zip(mem1[1:], mem2[1:])):
        ...
        print("%d\t%d\t%d\t%d\t%.2f" % (i+1, len(set1), len(set2), common, ratio))
